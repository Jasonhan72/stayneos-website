...
export default function PropertyCard({ property }: PropertyCardProps) {
  ...
  // 只在客户端渲染收藏按钮，避免 hydration mismatch
  const renderFavoriteButton = () => {
    if (!isClient) {
      return (
        <div className="absolute top-3 right-3 p-2.5 rounded-full bg-white/95 backdrop-blur-sm min-w-[44px] min-h-[44px] flex items-center justify-center">
          {/* 服务端渲染时显示占位符 */}
