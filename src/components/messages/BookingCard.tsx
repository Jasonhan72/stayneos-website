...
export default function BookingCard({ conversation, visible, onClose, t, forceOverlay }: { conversation: ApiConversation; visible: boolean; onClose: () => void; t: (k: string, d: string) => string; forceOverlay?: boolean; }) {
  return <>
    {!forceOverlay ? <aside className={cn("hidden border-l border-neutral-200 bg-white transition-all duration-200 xl:block", visible ? "w-[360px]" : "w-0 overflow-hidden border-l-0")}><div className={cn("w-[360px]", !visible && "hidden")}><Content conversation={conversation} t={t} /></div></aside> : null}
    {forceOverlay ? (
      <div className={cn("fixed inset-0 z-50 flex items-center justify-center p-4", visible ? "pointer-events-auto" : "pointer-events-none")}><div onClick={onClose} className={cn("absolute inset-0 bg-black/40 transition-opacity", visible ? "opacity-100" : "opacity-0")} /><div className={cn("relative max-h-[80vh] w-full max-w-[420px] overflow-y-auto rounded-[28px] bg-white shadow-2xl transition-all duration-200", visible ? "scale-100 opacity-100" : "scale-95 opacity-0")}><div className="sticky top-0 z-10 flex items-center justify-between border-b border-neutral-100 bg-white px-5 py-4 rounded-t-[28px]"><h3 className="text-lg font-semibold">{t('messages.reservationDetails', 'Reservation details')}</h3><button onClick={onClose} className="rounded-full p-2 hover:bg-neutral-100"><X className="h-5 w-5" /></button></div><Content conversation={conversation} t={t} /></div></div>
    ) : (
...
function Content({ conversation, t }: { conversation: ApiConversation; t: (k: string, d: string) => string }) {
  ...
  return <div className="p-5">
    <div className="overflow-hidden rounded-[28px] border border-neutral-200 bg-white shadow-sm">
      <div className="aspect-[4/3] bg-neutral-100">{property?.imageUrl ? <img src={property.imageUrl} alt={property.title || "Property"} loading="lazy" decoding="async" className="h-full w-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = "/images/wellesley-1607-living.jpg"; }} /> : <img src="/images/wellesley-1607-living.jpg" alt="StayNeos property" loading="lazy" decoding="async" className="h-full w-full object-cover" />}</div>
