...
function mockToPublic(p: typeof mockProperties[0]) {
  return {
    ...
    priceQuarterly: null,
    priceAnnual: null,
    defaultStayType: p.priceUnit === 'night' ? 'NIGHTLY' : 'MONTHLY',
    nightlyRate: p.priceUnit === 'night' ? p.price : undefined,
    monthlyRate: p.priceUnit === 'night' ? Math.floor(p.price * 30 * 0.8 / 100) * 100 : p.price,
    currency: 'CAD',
    images: p.images,
