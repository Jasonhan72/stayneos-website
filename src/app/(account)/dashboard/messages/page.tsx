...
export default function MessagesPage() {
  ...
  if (authLoading) return <PageShell><div className="flex h-full items-center justify-center"><div className="h-8 w-8 animate-spin rounded-full border-2 border-neutral-300 border-t-neutral-950" /></div></PageShell>;

  return <PageShell>
    <div className="flex h-full overflow-hidden rounded-none bg-white md:border-x md:border-neutral-200">
      <div className={`${selectedConv ? "hidden md:block" : "block"} w-full shrink-0 border-r border-neutral-200 md:w-[390px]`}><ConversationList conversations={conversations} selectedId={selectedId} unreadOnly={unreadOnly} loading={apiLoading} error={listError} onSelect={handleSelect} onToggleUnread={() => setUnreadOnly((v) => !v)} onRetry={() => fetchConversations()} t={t} /></div>
      <div className={`${selectedConv ? "flex" : "hidden md:flex"} min-w-0 flex-1`}>{selectedConv ? <ChatArea conversation={selectedConv} messages={messages} loading={msgsLoading} loadingOlder={loadingOlder} hasMore={!!cursor} refreshing={refreshing} error={threadError} onBack={() => setSelectedId(null)} onLoadOlder={() => selectedId && fetchMessages(selectedId, { older: true })} onSend={handleSendMessage} onToggleDetails={() => setDetailsVisible((v) => !v)} detailsVisible={detailsVisible} t={t} currentUserId={user?.id || ""} /> : <DesktopEmpty />}</div>
      {selectedConv ? <BookingCard conversation={selectedConv} visible={detailsVisible} onClose={() => setDetailsVisible(false)} t={t} /> : null}
    </div>
  </PageShell>;
}

function PageShell({ children }: { children: React.ReactNode }) { return <main className="h-[calc(100vh-64px)] overflow-hidden bg-white md:h-[calc(100vh-80px)]">{children}</main>; }
function DesktopEmpty() { return <div className="hidden h-full flex-1 flex-col items-center justify-center bg-[#F7F7F7] text-center md:flex"><div className="mb-5 flex h-20 w-20 items-center justify-center rounded-[28px] bg-white shadow-sm"><MessageCircle className="h-9 w-9 text-neutral-300" /></div><h2 className="text-xl font-semibold text-neutral-950">Select a conversation</h2><p className="mt-2 max-w-sm text-sm leading-6 text-neutral-500">Choose a guest or host message to see the full thread and reservation details.</p></div>; }
