  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 px-6 py-12 lg:px-12">
      <div className="max-w-3xl">
        <h1 className="text-3xl font-semibold tracking-tight text-neutral-900">{L("通知设置", "Notifications", "Notifications")}</h1>
        <p className="mt-3 text-sm leading-6 text-neutral-500">{L("选择您想通过邮件和短信接收的更新。", "Choose which updates you want to receive by email and SMS.", "Choisissez les mises à jour que vous souhaitez recevoir par e-mail et SMS.")}</p>
      </div>
      <Section icon={<Mail className="h-5 w-5" />} title={L("邮件通知", "Email notifications", "Notifications par e-mail")}>
        <div className="divide-y divide-neutral-100">{emailToggles.map((item) => <div key={item.key} className="flex items-center justify-between gap-4 py-4"><div><div className="text-sm font-medium text-neutral-900">{L(item.labelZh, item.labelEn, item.labelFr)}</div><div className="text-sm text-neutral-500">{L(item.descZh, item.descEn, item.descFr)}</div></div><ToggleSwitch checked={toggles[item.key]} onChange={() => toggle(item.key)} disabled={savingKey === item.key} /></div>)}</div>
      </Section>
      <Section icon={<Smartphone className="h-5 w-5" />} title={L("短信通知", "SMS notifications", "Notifications par SMS")}>
        <div className="divide-y divide-neutral-100">{smsToggles.map((item) => <div key={item.key} className="flex items-center justify-between gap-4 py-4"><div><div className="text-sm font-medium text-neutral-900">{L(item.labelZh, item.labelEn, item.labelFr)}</div><div className="text-sm text-neutral-500">{L(item.descZh, item.descEn, item.descFr)}</div></div><ToggleSwitch checked={toggles[item.key]} onChange={() => toggle(item.key)} disabled={savingKey === item.key} /></div>)}</div>
      </Section>
    </div>
  );
}

function Section({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) { return <div className="rounded-3xl border border-neutral-200 p-6"><div className="mb-4 flex items-center gap-3"><span className="text-neutral-600">{icon}</span><h2 className="text-base font-semibold text-neutral-900">{title}</h2></div>{children}</div>; }
