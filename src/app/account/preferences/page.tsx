...
export default function PreferencesPage() {
  ...
  const sections = useMemo(() => ([
    { id: 'general', title: L('通用', 'General', 'Général'), rows: [
      { key: 'language', icon: <Globe className="w-5 h-5" />, label: L('语言', 'Language', 'Langue'), options: [{ value: 'en', label: 'English' }, { value: 'zh', label: '中文' }, { value: 'fr', label: 'Français' }] },
      { key: 'currency', icon: <DollarSign className="w-5 h-5" />, label: L('货币', 'Currency', 'Devise'), options: [{ value: 'CAD', label: 'CAD ($)' }, { value: 'USD', label: 'USD ($)' }, { value: 'EUR', label: 'EUR (€)' }] },
      { key: 'theme', icon: <Moon className="w-5 h-5" />, label: L('主题', 'Theme', 'Thème'), options: [{ value: 'system', label: L('跟随系统', 'System', 'Système') }, { value: 'light', label: L('浅色', 'Light', 'Clair') }, { value: 'dark', label: L('深色', 'Dark', 'Sombre') }] },
      { key: 'contentDensity', icon: <Ruler className="w-5 h-5" />, label: L('内容密度', 'Content density', 'Densité du contenu'), options: [{ value: 'comfortable', label: L('舒适', 'Comfortable', 'Confortable') }, { value: 'compact', label: L('紧凑', 'Compact', 'Compact') }] },
    ] },
    { id: 'personalization', title: L('个性化', 'Personalization', 'Personnalisation'), rows: [
      { key: 'dateFormat', icon: <Layout className="w-5 h-5" />, label: L('日期格式', 'Date format', 'Format de date'), options: [{ value: 'MMM D, YYYY', label: 'Jan 5, 2026' }, { value: 'D MMM YYYY', label: '5 Jan 2026' }, { value: 'YYYY-MM-DD', label: '2026-01-05' }, { value: 'DD/MM/YYYY', label: '05/01/2026' }] },
      { key: 'firstDayOfWeek', icon: <Sparkles className="w-5 h-5" />, label: L('每周起始日', 'First day of week', 'Premier jour de la semaine'), options: [{ value: 'monday', label: L('周一', 'Monday', 'Lundi') }, { value: 'sunday', label: L('周日', 'Sunday', 'Dimanche') }] },
      { key: 'messageSort', icon: <MessageSquare className="w-5 h-5" />, label: L('消息排序', 'Message sort order', 'Tri des messages'), options: [{ value: 'newest', label: L('最新在前', 'Newest first', 'Plus récents d\'abord') }, { value: 'oldest', label: L('最早在前', 'Oldest first', 'Plus anciens d\'abord') }] },
    ] },
  ]), [L]);
